...
/// A stateless eidget that represents the general sensorboard Page.
/// 
/// This page displays the average air pressure measurement, also the navigations to the 4 Sensorboards
/// It also includes navigation, status, and notifications management.
class SensorBoardPage extends StatefulWidget {
  const SensorBoardPage({Key? key}) : super(key: key);

  @override
  State<SensorBoardPage> createState() => _SensorBoardPageState();
}

class _SensorBoardPageState extends State<SensorBoardPage> {
  @override
    ...
    return Scaffold(
      body: Row(
        children: [
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                _buildNavItem(context, route: '/sensorboard/sensor_board_1', icon: Icons.developer_board, label: 'Board 1'),
                _buildNavItem(context, route: '/sensorboard/sensor_board_2', icon: Icons.developer_board, label: 'Board 2'),
                _buildNavItem(context, route: '/sensorboard/sensor_board_3', icon: Icons.developer_board, label: 'Board 3'),
                _buildNavItem(context, route: '/sensorboard/sensor_board_4', icon: Icons.developer_board, label: 'Board 4'),
                const Divider(color: Colors.white),
                    ...
                    children: [
                      ...
                      Consumer<DataManager>(
                        builder: (context, manager, _) {
                          String formatValue(double value) => value.toStringAsFixed(3);

                          Color determineColor(String sensorName, double value) {
                            ...
                            if (value < config.extremMin || value > config.extremMax) {
                              return Colors.red;
                            } else if ((value >= config.extremMin && value < config.yellowLower) ||
                                (value > config.yellowUpper && value <= config.extremMax)) {
                              return Colors.red;
                            } else if ((value >= config.yellowLower && value < config.normalMin) ||
                                (value > config.normalMax && value <= config.yellowUpper)) {
                          ...
                          final press1 = double.tryParse(manager.getLatestValue('board1_amb_press')) ?? 0.0;
                          final press2 = double.tryParse(manager.getLatestValue('board2_amb_press')) ?? 0.0;
                          final press3 = double.tryParse(manager.getLatestValue('board3_amb_press')) ?? 0.0;
                          final press4 = double.tryParse(manager.getLatestValue('board4_amb_press')) ?? 0.0;

                          
                          final avgPress = (press1 + press2 + press3 + press4) / 4;

                          return Padding(
                            padding: const EdgeInsets.all(16.0),
                            child: _buildField(
                              'Avg Air Press',
                              formatValue(avgPress),
                              determineColor('Air Pressure (Output, g)', avgPress),
                            ),
                          );
                        },
                      ),
                      ...
                      Expanded(
                        child: GridView.count(
                          ...
                          childAspectRatio: 4,
                          children: [
                            _buildSensorBox(context, 'Sensor Board 1', '/sensorboard/sensor_board_1'),
                            _buildSensorBox(context, 'Sensor Board 2', '/sensorboard/sensor_board_2'),
                            _buildSensorBox(context, 'Sensor Board 3', '/sensorboard/sensor_board_3'),
                            _buildSensorBox(context, 'Sensor Board 4', '/sensorboard/sensor_board_4'),
                          ],
                        ),
                      ),
                    ],
          ...
          // right collumn for notification and status
          Container(
            width: 300,
  ...
  Widget _buildField(String label, String value, Color color) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(label, style: const TextStyle(color: Color.fromARGB(255, 0, 0, 0), fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          Text(value, style: const TextStyle(color: Color.fromARGB(179, 0, 0, 0), fontSize: 16)),
        ],
      ),
    );
  }
